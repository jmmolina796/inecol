(function($){
	$(function(){
			...
			$(".mt-form").each(function(){
				if(!$(this).hasClass("mt-check"))
				{
					...
					$(this).find(">div.slct>div").each(function(){
						...
						$(this).find("select.sgl, select.mlt").data({"label":$(this).find("select.sgl").attr("data-label"),"require":$(this).find("select.sgl, select.mlt").attr("data-require")});
		...
		$.fn.mtValidate = function(){
			...
			{
				$(elementShow).velocity("scroll", {offset:"-20px"});
			}
			return verify.show();
		};
		...
		$.fn.mtError = function(){
			...
			$(this).velocity("scroll", {offset:"-20px"});
			var elementObject = {
				parent: $(this),
				html: element,
				modificarCss: function(){
					this.parent.css({"border-bottom":"#F44336 solid 2px"});
					this.html.css({"color":"#F44336"});
				}
			};
